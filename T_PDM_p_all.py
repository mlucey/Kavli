...
def neural_network_mod():
    ...
    net = tf.layers.dense(net, 24, activation=tf.nn.sigmoid)
...
def train(log_likelihood,train_op,n_epoch):
    ...
    plt.savefig('Plots/loss_function_t_all.pdf')
    return train_loss
...
def plot_pdfs(pred_means,pred_weights,pred_std,num=10,train=True):
    ...
    plt.tight_layout()
    if train:
        plt.savefig('Plots/train_pdfs_t_all.pdf')
    else:
        plt.savefig('Plots/test_pdfs_t_all.pdf')
    #plt.show()
...
def plot_pred_mean(pred_means,pred_weights,pred_std,ymax,ymin,y_train,select='no'):
    ...
    plt.tight_layout()
    if train:
        plt.savefig('Plots/train_scatter_t_all.pdf')
    else:
        plt.savefig('Plots/test_scatter_t_all.pdf')
    #plt.show()
...
n_epochs = 1000000 #1500 #1000000 #1000 #20000 #20000
