...
def GenData_lamost(fileIn = 'lamost_wise_gaia_PS1_2mass.fits'):
    ...
    y_train_all = np.array(al[param[1]]).T
    ...
    t = Table.read('Tables/test_all_logg_teff.fits')
    inds = np.where((t['logg_phot_1']<3.5) & (np.logical_and(t['teff_phot_1']>2500,t['teff_phot_1']<5500)))[0]
    ...
    x_train_shuffled = x_train_rescaled[TrainshuffleOrder]
    y_train_shuffled = y_train_rescaled[TrainshuffleOrder]
    X_test =x_train_shuffled[:num_test]
    y_test = y_train_shuffled[:num_test]
    test_tinds = tinds[:num_test]
...
def train(log_likelihood,train_op,n_epoch):
    ...
    plt.savefig('Plots/loss_function_g.pdf')
    return train_loss
...
def plot_pdfs(pred_means,pred_weights,pred_std,num=10,train=True):
    ...
    if train:
        plt.savefig('Plots/train_pdfs_g_2.pdf')
    else:
        plt.savefig('Plots/test_pdfs_g_2.pdf')
    plt.tight_layout()
    #plt.show()
...
def plot_pred_mean(pred_means,pred_weights,pred_std,ymax,ymin,y_train,select='no',train=True):
    ...
    if train:
        plt.savefig('Plots/train_scatter_g_2.pdf')
    else:
        plt.savefig('Plots/test_scatter_g_2.pdf')
    plt.tight_layout()
    #plt.show()
...
n_epochs = 1500 #1000000 #1000000 #1000 #20000 #20000
