...
def GenData_lamost(fileIn = 'rgb_p.fits'):
    ...
    inds_train = np.where(al['s_n'][TrainshuffleOrder]>75)[0][:num_train]
    train_tinds = tinds[inds_train]

    test_tinds = np.array(list(set(range(len(x_train_shuffled)))-set(inds_train)),dtype='int')

    X_train = x_train_shuffled[inds_train]  # color mag
    X_test = x_train_shuffled[test_tinds][:num_test]
    y_train = y_train_shuffled[inds_train]  # spec z
    y_test = y_train_shuffled[test_tinds][: num_test]  # spec z
    #classy = classy_shuffled[:num_train]
    params = params_shuffled[inds_train]
    classy = classy[inds_train]
    test_tinds = tinds[test_tinds][:num_test]

    return X_train, y_train, X_test, y_test, classy, params, ymax, ymin, xmax, xmin, xmax_a, xmin_a, train_tinds, test_tinds
...
def mixture_model(X,Y=None,learning_rate=1e-3,decay_rate=.95,step=1000,train=True):
    if train:
...
def train(log_likelihood,train_op,n_epoch):
    ...
    #plt.show()
    plt.savefig('Plots/loss_function_nu.pdf')
    return train_loss
...
def plot_pdfs(pred_means,pred_weights,pred_std,num=10,train=True):
    ...
    plt.tight_layout()
    if train:
        plt.savefig('Plots/train_pdfs_nu.pdf')
    else:
        plt.savefig('Plots/test_pdfs_nu.pdf')
...
def plot_pred_mean(pred_means,pred_weights,pred_std,ymax,ymin,y_train,select='no'):
    ...
    #plt.show()
    if train:
        plt.savefig('Plots/train_scatter_nu.pdf')
    else:
        plt.savefig('Plots/test_scatter_nu.pdf')
...
save_mod = 'nu_Models/lr'+str(learning_rate)+'_dr'+str(decay_rate)+'_step'+str(step)+'_ne'+str(n_epochs)+'_k'+str(K)+'_nt'+str(num_train)
...
def load_data(filein='../../Kavli/rgb_p_rev_phot_phot.fits',y_exist=True):
...
def save_inf(pred_means,pred_weights,pred_std,filein='lamost_rc_qual.fits',test=True,train=False,y_train=y_train):
    y_pred = np.sum(pred_means*pred_weights, axis = 1)
    y_pred_std = np.sum(pred_std*pred_weights, axis = 1)
    y_pred = (ymax - ymin)*(y_pred)+ymin
    y_pred_std = (ymax - ymin)*(y_pred_std)
    if test:
        al = Table.read(filein)
        al = al[test_inds]
        al['Delatnu_phot'] = y_pred
        al['Deltanu_phot_error'] = y_pred_std
        al.write('Tables/test_rc_nu.fits',overwrite=True)
    if train:
...
"""
rc_x= load_data(y_exist=False)
rc_weights, rc_means, rc_std = testing(rc_x)
